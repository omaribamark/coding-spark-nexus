...
router.post('/resend-2fa', async (req, res) => {
  try {
    ...
    // Get user email and test status
    const userResult = await db.query(
      'SELECT email, username, is_playstore_test FROM hakikisha.users WHERE id = $1',
      [targetUserId]
    );
    ...
    const isTestUser = user.is_playstore_test === true || shouldBypassOTP(user.email);
    if (isTestUser) {
      console.log(`🔓 SKIPPING 2FA OTP for test user: ${user.email} - No OTP will be sent`);
      return res.json({
        success: true,
    ...
    // Resend OTP for regular users only
