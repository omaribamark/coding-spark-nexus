...
const ClaimDetailsScreen = () => {
  ...
  // UPDATED: Extract links and images with proper mediaUrl and userImages support
  const extractLinks = (claim: any) => {
    console.log('Extracting links and images from claim:', {
      ...
      userImages: claim.userImages, // NEW: Check userImages array
      mediaUrl: claim.mediaUrl, // NEW: Check mediaUrl field
      mediaType: claim.mediaType, // NEW: Check mediaType
    ...
    // PRIORITY 1: Check userImages array (from backend)
    ...
    // PRIORITY 2: Check mediaUrl for images
    else if (claim.mediaUrl && claim.mediaType === 'media') {
    ...
    // PRIORITY 3: Check legacy image fields
    ...
    console.log('Extracted media:', { 
      ...
      userImages, 
      verdictImageUrl,
      userImagesSource: claim.userImages ? 'from_userImages' : 
                       claim.mediaUrl ? 'from_mediaUrl' : 
                       claim.user_images ? 'from_verdict' : 
                       claim.images ? 'from_claim' : 'from_imageUrl'
    });

    return {videoLink, sourceLink, userImages, verdictImageUrl};
  };

  // UPDATED: Render user images with better styling
    ...
    return (
      <View className="mt-4">
        <Text className={`${isDark ? 'text-gray-400' : 'text-gray-600'} text-xs font-pmedium mb-3`}>
          📸 Claim Evidence Images ({images.length})
        </Text>
        <ScrollView 
          horizontal 
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="p-6">
          ...
          <View className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-5 mb-6 border`}>
            ...
            <View className="mb-4">
              <Text className={`${isDark ? 'text-gray-400' : 'text-gray-500'} font-pmedium text-sm mb-3`}>VERDICT ANALYSIS</Text>
              <Text className={`${isDark ? 'text-gray-500' : 'text-gray-400'} text-xs mb-3 italic`}>
                ℹ️ The analysis below is based on the claim statement and evidence shown above.
              </Text>
            </View>
