...
const isPlayStoreTestUser = async (email) => {
  try {
    console.log('🔍 Checking if user is Play Store test user:', email);
    const result = await db.query(
      'SELECT is_playstore_test FROM hakikisha.users WHERE LOWER(email) = $1',
      [email.toLowerCase()]
    );
    
    const isTest = result.rows.length > 0 && result.rows[0].is_playstore_test === true;
    console.log('🔍 Play Store test user check result:', { email, isTest });
    return isTest;
  } catch (error) {
    console.error('❌ Error checking Play Store test user:', error);
    return false;
  }
};

// NEW: Direct database check for test users by user ID
const isPlayStoreTestUserById = async (userId) => {
  try {
    console.log('🔍 Checking if user is Play Store test user by ID:', userId);
    const result = await db.query(
    ...
    const isTest = result.rows.length > 0 && result.rows[0].is_playstore_test === true;
    console.log('🔍 Play Store test user by ID check result:', { userId, isTest });
    return isTest;
  } catch (error) {
    console.error('❌ Error checking Play Store test user by ID:', error);
    return false;
  }
};
...
// ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
// Auto-verify and auto-approve bypass users
const isBypassUser = shouldBypassOTP(email);
const isTestUser = isBypassUser; // Mark all bypass users as test users
const registrationStatus = isBypassUser ? 'approved' : (role === 'fact_checker' ? 'pending' : 'approved');
const isVerified = isBypassUser; // Bypass users are auto-verified

console.log('🔍 Registration - User bypass status:', {
  email,
  isBypassUser,
  isTestUser,
  registrationStatus,
...
// ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
    // Skip email verification for bypass users
    if (!isBypassUser) {
      // Generate and send email verification OTP for regular users
      try {
      ...
      console.log(`🔓 OTP bypass for Google Play Store user registration: ${user.email}`);
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
  ...
  try {
    ...
    const userResult = await db.query(
      `SELECT id, email, username, password_hash, role, is_verified, registration_status, 
              two_factor_enabled, status, login_count, last_login, is_playstore_test
    ...
    if (!isBypassUser && !user.is_verified) {
      // ENFORCE EMAIL VERIFICATION FOR REGULAR USERS BEFORE FIRST LOGIN
    ...
    // ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
    // Skip 2FA for bypass users even if it's enabled
    if (user.two_factor_enabled && !isBypassUser) {
      const otp = safeGenerateOTP();
    ...
    // ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
    if (isBypassUser) {
      console.log(`🔓 OTP bypass login successful for Google Play Store user: ${user.email}`);
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
...
const verify2FA = async (req, res) => {
  try {
    ...
    // ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
    // Check if this is a Play Store test user
    const userCheck = await db.query(
      'SELECT email, is_playstore_test FROM hakikisha.users WHERE id = $1', 
      [userId]
    );
    ...
    if (userCheck.rows.length > 0) {
      ...
      if (user.is_playstore_test === true) {
        console.log(`🔓 FULL OTP BYPASS for Play Store test user in 2FA: ${user.email}`);
        
        const userResult = await db.query(
          'SELECT id, email, username, role, is_verified, registration_status, two_factor_enabled, is_playstore_test FROM hakikisha.users WHERE id = $1',
          [userId]
        );
        ...
        const refreshToken = jwt.sign(
          { userId: userData.id, email: userData.email, type: 'refresh' },
          JWT_SECRET,
          { expiresIn: REFRESH_TOKEN_EXPIRES_IN }
        );

        // Create user session
        await createUserSession(userData, token, refreshToken);

        return res.json({
          success: true,
          message: 'Login successful (Play Store test account - OTP bypassed)',
          token,
          refreshToken,
          user: {
            id: userData.id,
            ...
            is_playstore_test: userData.is_playstore_test
          }
        });
      }
      ...
      if (shouldBypassOTP(user.email)) {
        console.log(`🔓 OTP bypass for Google Play Store user in 2FA: ${user.email}`);
...
const forgotPassword = async (req, res) => {
  try {
    ...
    // ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
    // Check if this is a Play Store test user
    const isTestUser = await isPlayStoreTestUser(email);
    if (isTestUser) {
      console.log(`🔓 Password reset OTP bypass for Play Store test user: ${email}`);
      return res.json({
        success: true,
        message: 'Password reset not required for test accounts. Use default password.',
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (isTestUser) {
      ...
      return res.status(400).json({
        success: false,
        error: 'Password reset not allowed for test accounts. Use default password.',
        code: 'TEST_ACCOUNT_RESTRICTION'
      });
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
...
const refreshToken = async (req, res) => {
  try {
    ...
    const userResult = await db.query(
      'SELECT id, email, role, status, is_verified, is_playstore_test FROM hakikisha.users WHERE id = $1',
      [decoded.userId]
    );
    ...
    // ==================== GOOGLE PLAY STORE OTP BYPASS - START ====================
    // Skip email verification check for test users
    if (user.is_playstore_test !== true && !user.is_verified) {
      return res.status(403).json({
        success: false,
  ...
  try {
    ...
    const result = await db.query(
      `SELECT 
        ...
        u.created_at, u.last_login, u.login_count, u.updated_at, u.is_playstore_test,
...
const checkAuth = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT 
        u.id, u.email, u.username, u.role, u.is_verified, u.registration_status, u.is_playstore_test,
        COALESCE(up.total_points, 0) as points,
        COALESCE(up.current_streak, 0) as current_streak,
        COALESCE(up.longest_streak, 0) as longest_streak
       FROM hakikisha.users u
    ...
    console.log('Auth profile with points:', { 
      points, 
      currentStreak, 
      longestStreak,
      is_playstore_test: user.is_playstore_test 
...
const verifyEmail = async (req, res) => {
  try {
    ...
    if (userCheck.rows.length > 0 && userCheck.rows[0].is_playstore_test === true) {
      console.log(`🔓 Email verification bypass for Play Store test user: ${userId}`);
      ...
      return res.json({
        success: true,
        message: 'Email verified successfully (Play Store test account).'
      });
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
...
const resendVerificationCode = async (req, res) => {
  try {
    ...
    const isTestUser = await isPlayStoreTestUser(email);
    if (isTestUser) {
      console.log(`🔓 Email verification resend bypass for Play Store test user: ${email}`);
      return res.json({
        success: true,
        message: 'Test account already verified.',
        is_playstore_test: true
      });
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
...
const resend2FACode = async (req, res) => {
  try {
    ...
    const isTestUser = await isPlayStoreTestUser(email);
    if (isTestUser) {
      console.log(`🔓 2FA resend bypass for Play Store test user: ${email}`);
      return res.json({
        success: true,
        message: '2FA not required for test accounts.',
        is_playstore_test: true
      });
    }
    // ==================== GOOGLE PLAY STORE OTP BYPASS - END ====================
...
// NEW: Admin endpoint to delete Play Store test users
const deletePlayStoreTestUsers = async (req, res) => {
  try {
    console.log('Delete Play Store Test Users Request');
    ...
    const deletedUsers = await DatabaseInitializer.deletePlayStoreTestUsers();

    res.json({
      success: true,
      message: 'Play Store test users deleted successfully',
      deleted_users: deletedUsers
    });
  } catch (error) {
    console.error('Delete Play Store test users error:', error);
    logger.error('Delete Play Store test users error:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to delete Play Store test users',
      code: 'SERVER_ERROR'
    });
  }
};
