...
class ClaimController {
  async submitClaim(req, res) {
    try {
      ...
      // Handle image storage in database if base64 image is provided
      let mediaUrl = null;
      let mediaType = 'text';
      
      if (imageUrl && imageUrl.startsWith('data:image/')) {
        console.log('Processing base64 image for database storage...');
        ...
        try {
          ...
          if (matches && matches.length === 3) {
            ...
            const filename = `claim-${claimId}-${uuidv4()}.${fileExtension}`;
            ...
            const mediaResult = await db.query(
              ...
              [filename, filename, mimeType, fileSize, base64Data, req.user.userId, 'claim']
            );

            const mediaId = mediaResult.rows[0].id;
            
            // Construct the URL to retrieve the image (PUBLIC URL - accessible to all users)
            mediaUrl = `${req.protocol}://${req.get('host')}/api/v1/upload/images/${mediaId}`;
            mediaType = 'image';
            
            console.log('Image stored in database successfully. Media ID:', mediaId);
            console.log('Public image URL:', mediaUrl);
          } else {
            console.error('Invalid base64 image format');
          }
        } catch (imageError) {
          console.error('Failed to store image in database:', imageError);
          // Continue with claim submission even if image storage fails
        }
      } else if (videoLink) {
        mediaUrl = videoLink;
        mediaType = 'video';
  ...
  async uploadEvidence(req, res) {
    try {
      ...
      const filename = `evidence-${uuidv4()}-${req.file.originalname}`;
      ...
      const result = await db.query(
        `INSERT INTO hakikisha.media_files 
         ...
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING id, filename`,
        [filename, req.file.originalname, req.file.mimetype, fileSize, base64Data, req.user.userId, 'claim']
      );

      const mediaId = result.rows[0].id;
      const fileUrl = `${req.protocol}://${req.get('host')}/api/v1/upload/images/${mediaId}`;

      console.log('Evidence file stored in database:', filename);
