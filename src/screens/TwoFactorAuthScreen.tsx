...
const TwoFactorAuthScreen = () => {
  ...
  // NEW: Auto-bypass for test users
  useEffect(() => {
// In TwoFactorAuthScreen.tsx - Update the autoBypass2FA function
const autoBypass2FA = async () => {
  if (TEST_USERS.includes(email.toLowerCase())) {
    console.log('🚀 Test user detected, bypassing 2FA:', email);
    setLoading(true);
    
    try {
      // FIX: Use real authentication instead of mock tokens
      console.log('🔄 Getting real authentication for test user...');
      
      // Call login again to get real tokens
      const loginData = {
        identifier: email,
        password: getTestUserPassword(email) // You'll need to implement this
      };
      
      const loginResponse = await authService.login(loginData);
      
      if (loginResponse.success && loginResponse.token) {
        console.log('✅ Got real authentication for test user');
        ...
        console.log('✅ 2FA bypassed for test user, redirecting...');
        ...
        throw new Error('Failed to get real authentication');
      }
      
    } catch (error) {
      console.error('❌ Error in 2FA bypass:', error);
      // Fallback to manual verification
      Alert.alert('Auto-login Failed', 'Please enter the 2FA code manually');
      setLoading(false);
    }
  }
};

// Helper function to get test user passwords
const getTestUserPassword = (email: string): string => {
  const passwords: { [key: string]: string } = {
    'admin.bypass@hakikisha.com': 'AdminBypass2024!',
    'factchecker.bypass@hakikisha.com': 'FactCheckerBypass2024!',
    'user.normal@hakikisha.com': 'UserNormal2024!'
  };
  return passwords[email] || 'DefaultPassword2024!';
};

    autoBypass2FA();
  }, [email, userId, role, navigation]);
