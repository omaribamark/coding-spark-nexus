...
const AdminDashboardScreen = (props: Props) => {
  ...
  // NEW: Function to render claim images
  const renderClaimImages = (claim: any) => {
    const imagesToDisplay: string[] = [];
    
    // Priority 1: mediaUrl from database (new system)
    ...
    if (claim.images && claim.images.length > 0) {
      claim.images.forEach((img: string) => {
    ...
    // Priority 3: userImages array
    if (claim.userImages && claim.userImages.length > 0) {
      claim.userImages.forEach((img: string) => {
        if (!imagesToDisplay.includes(img)) {
          imagesToDisplay.push(img);
        }
      });
    ...
    // Priority 4: single imageUrl (legacy)
  ...
  const handleRegisterUser = async () => {
    ...
    try {
      ...
      if (registerPhoneNumber) {
        userData.phone = `+${selectedCountry.callingCode}${registerPhoneNumber}`;
      }
  ...
  // UPDATED: Claim Details Modal with image support
